...
if model_type == "Logistic Regression":
    ...
    model = LogisticRegression(penalty=penalty, C=C, max_iter=1000, solver='lbfgs')
else:
...
if 'Genre' in df.columns:
    ...
    df['Genre'].value_counts().plot(kind='bar', ax=ax, color='coral')
...
# Clustering
st.subheader("🔍 Clustering Songs by Similarity")
kmeans = KMeans(n_clusters=5, random_state=42)
df['Cluster'] = kmeans.fit_predict(X)
st.write("Songs are clustered into groups. Select a cluster to explore:")
cluster_num = st.selectbox("Select Cluster Number", range(5))
